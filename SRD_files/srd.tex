...
\hline
\hline
\textbf{Load game}    & Avoir une configuration enregistrée (pc, base de données). L'utilisateur doit être enregistré dans la base de données   & Néant  & Configuration de la partie via le fichier de sauvegarde.  & Fichier corrompu, impossible de charger le fichier de sauvegarde. Fichier n'existe pas/introuvable\\
\hline
\hline
\textbf{Create game}    & L'utilisateur doit être enregistré dans la base de données  & Possibilité de sauvegarder les options requises  & L'utilisateur peut lancer une partie après avoir rempli les conditions minimales  & Oubli d'une option (affichage d'un message d'erreur) \\
\hline
\hline
...
\hline
\hline
\textbf{Chat}     & L'utilisateur doit être enregistré dans la base de données   & Le serveur effectue les liaisons entre les utilisateurs  & Envoyer et recevoir des messages avec des personnes qui sont dans le salon.  & Néant \\
\hline
\hline
\textbf{Add friend}    & L'utilisateur doit être enregistré dans la base de données   & Ajout d'amis dans la base de données.  & Entrer le pseudo d’un ami. Le système va rechercher dans la base de données si le pseudo existe et l’ajouter.  & Ajouter un ami qui n’existe pas (affiche une erreur).\\
\hline
\hline
