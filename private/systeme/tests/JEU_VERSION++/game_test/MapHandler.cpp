...
void MapHandler::update_server(MapObject::type typ,int i,std::string* to_fill){
    ...
    add_object_server(typ,i,to_fill);
}

void MapHandler::add_object_server(MapObject::type typ,int i, std::string *to_fill){
        // spawn a new object
    ...
    if(typ == MapObject::star )
        ...
        int x = rand() % (field_bounds.width() - 1) + 1;
        std::string str_x = std::to_string(x);
        enemy_ships_set.push_back(new EnemyShip(x, 0, {{10 - 1, 5},{3,      2}}, '%', enemyStartHp,t + rand() % 100, enemyStartProjectileDamage));
        to_fill->append("E|");
        to_fill->append(str_x);
        to_fill->append("&");
        enemyCount++;
