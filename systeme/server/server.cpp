...
void Server::catchInput(char* input) {
    ...
    long unsigned int  i = processId.rfind(Constante::DELIMITEUR);
...
void Server::initConnexions(){
    ...
    int fd;
    char proc_id[Constante::CHAR_SIZE], connex_pipe_path[Constante::CHAR_SIZE];
    ...
    while (true){
        ...
        if (fd != -1){
            ssize_t val = read(fd,proc_id,Constante::CHAR_SIZE);
            if (val == -1)
            {
...
void Server::client_exit(std::string *pid){
    ...
    for(size_t i = 0; i < _pipe_running.size();i++){
        if(_pipe_running.at(i)->in_game && strcmp(_pipe_running.at(i)->pid, pid->c_str()) == 0 ){
            ...
            _pipe_running.erase(_pipe_running.begin()+long(i));
            break;
        }
    }
    mtx.unlock();
...
void Server::launch_game(Game_settings* sett_game){
    ...
    for(size_t i =0; i < _pipe_running.size(); i++){
        if(strcmp(_pipe_running.at(i)->pid,sett_game->pid)== 0){
           _pipe_running.at(i)->in_game = false;
        }
    }
    mtx.unlock();
}
...
int Server::read_game_input(char * pipe){
    ...
    if (fd != -1){
        ssize_t val = read(fd,&message,sizeof(int)); 
