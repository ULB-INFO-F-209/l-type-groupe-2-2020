...
void MapHandler::update_server(MapObject::type typ,int t,std::string* to_fill){
    ...
    add_object_server(typ,t,to_fill);
}

void MapHandler::add_object_server(MapObject::type typ,int t, std::string *to_fill){
        // spawn a new object
    ...
    if(typ == MapObject::star )
        ...
        std::string str_x = std::to_string(x);
        std::string str_tick = std::to_string(enemy_tick);
...
void MapHandler::bossShoot_server(int tick,std::string *to_fill) {
    for (auto & b : boss_set) {
        if (tick== b->getShootTime()+50){
...
void MapHandler::enemyShoot_server(int tick,std::string *to_fill) {
    for (auto & i : enemy_ships_set) {
        if (tick== i->getShootTime()+100){
...
void MapHandler::checkCollision_server(int t, bool friendlyFire,std::string * to_fill) {
    ...
    for(size_t i=0; i < projectilesEnemy_set.size(); i++){
        for(size_t j=0; j < projectilesEnemy_set.size(); j++){
            ...
            || projectiles_set.at(i)->getPos().y+1 == projectilesEnemy_set.at(j)->getPos().y)) {
                to_fill->append("C|P|P|"); to_fill->append(std::to_string(i));
                to_fill->append("|"); to_fill->append(std::to_string(j));to_fill->append("&");
                projectiles_set.at(i)->touched(projectiles_set.at(i)->getHp());
                projectilesEnemy_set.at(j)->touched(projectilesEnemy_set.at(j)->getHp());
            }
        }
    }
    ...
    for(size_t i =0; i < enemy_ships_set.size(); i++){
        for(size_t j =0; j < projectiles_set.size(); j++){
            if(enemy_ships_set.at(i)->getBounds().contains(projectiles_set.at(j)->getPos()) && enemy_ships_set.at(i)->getHp()>0){
                to_fill->append("C|E|P|"); to_fill->append(std::to_string(i));
                to_fill->append("|"); to_fill->append(std::to_string(j));to_fill->append("&");
                enemy_ships_set.at(i)->touched(projectiles_set.at(j)->getDamage());
                projectiles_set.at(j)->touched(enemy_ships_set.at(i)->getDammage());
                if(player_ships_set.size() == 2){
                    ...
                    if(enemy_ships_set.at(i)->getHp()==0 && player_ships_set.at(projectiles_set.at(j)->getPlayer()-1)->getCurrentBonus()==lifeSteal && player_ships_set.at(projectiles_set.at(j)->getPlayer()-1)->getHp()<100){
                    ...
                    player_ships_set.at(0)->setScore(player_ships_set.at(0)->getScore() + 10);
                    if(enemy_ships_set.at(i)->getHp()==0 && player_ships_set.at(0)->getCurrentBonus()==lifeSteal && player_ships_set.at(0)->getHp()<100){
                        if ((player_ships_set.at(0)->getHp()+10) <= 100)
                            player_ships_set.at(0)->setHp(player_ships_set.at(0)->getHp()+10);
                        else player_ships_set.at(0)->setHp(100);
                    }
...
void MapHandler::spawnBonus_client(int x, int y,int rand_spawn_bonus) {
